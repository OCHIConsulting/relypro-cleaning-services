...
export function toHubSpotRecords(lead, config = {}) {
  const contact = compact({
    ...
    ...(lead.contact_method === 'email'
      ? { email: lead.contact_details, phone: lead.phone_number }
      : { phone: lead.contact_details })
  });
  const note = [
    `Enquiry reference: ${lead.enquiry_reference}`,
    `Received: ${lead.created_at}`,
    `Enquiry type: ${lead.kind === 'quote' ? 'Quote request' : 'Contact message'}`,
    `Name: ${lead.name}`,
    `Company or property name: ${lead.company_name || 'Not supplied'}`,
    `Preferred reply: ${lead.contact_method}`,
    `Contact details: ${lead.contact_details}`,
    `Phone number: ${lead.phone_number || (lead.contact_method === 'email' ? 'Not supplied' : lead.contact_details)}`,
    `Service: ${lead.service}`,
    `Postcode: ${lead.postcode || 'Not supplied'}`,
    ...(lead.kind === 'contact' && lead.subject
      ? [`Subject: ${lead.subject}`, `Message: ${lead.message || lead.property_summary}`]
      : [`What would you like cleaned?: ${lead.property_summary}`]),
    `Preferred date: ${lead.preferred_date || 'Not supplied'}`,
    `Landing page: ${lead.landing_page}`,
    `UTM source: ${lead.attribution?.utm_source || 'Not supplied'}`,
    `UTM medium: ${lead.attribution?.utm_medium || 'Not supplied'}`,
    `UTM campaign: ${lead.attribution?.utm_campaign || 'Not supplied'}`,
    `Privacy acknowledged: ${lead.privacy_acknowledged ? 'Yes' : 'No'}`,
